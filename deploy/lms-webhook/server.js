...
app.get('/healthz', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ok: true, server: APP_NAME, ip: APP_IP,
      redis: !!redisClient && isRedisConnected,
      uploadRoot: UPLOAD_ROOT, time: new Date().toISOString(),
    });
  } catch (error) {
    return res.status(503).json({
      ok: false, server: APP_NAME, ip: APP_IP,
...
app.use('/student/attempts', (req, res, next) => {
  ...
  if (redisClient && isRedisConnected) {
    ...
    redisClient.multi().incr(key).expire(key, 10).exec()
      .then(([count]) => {
...
function registerRoutes() {
  ...
  app.use(async (req, res, next) => {
    try { await trackOnlineUser(req); } catch (error) { console.error('[ONLINE-TRACKER] error:', error.message); }
    next();
  });
  ...
  app.get('/whoami', async (req, res) => {
    ...
    res.status(200).json({
      server: APP_NAME, ip: APP_IP, hostname: os.hostname(),
  ...
  if (AUTO_SUBMIT_MIDDLEWARE_ENABLED) app.use(autoSubmitMiddleware);
